
PUZZLE_WIDTH_STR =          ("Enter the width of the puzzle: ")
PUZZLE_HEIGHT_STR =         ("Enter the height of the puzzle: ")
DRAW_PUZZLE_STR =           ("Draw the puzzle by selecting the spaces it " + 
                             "occupies in the grid")
WORD_BANK_ENTRY_STR =       ("Enter a word in the word bank, or enter q to " +
                             "end the word bank generation: ")
WORD_BANK_WORD_ENTRY_STR =  ("{}. ")
CLUE_ENTRY_STR =            ("Enter the clue for the word marked with X's\n: ")
PRINTING_PUZZLE_STR =       ("This is what the puzzle looks like: ")
SOLVE_TIME_STR =            ("Solved in {} second{} and {} millisecond{}")
DISPLAYING_SOLUTIONS_STR =  ("Displaying solutions...")
NO_SOLUTIONS_STR =          ("No solutions found. Start new puzzle?(y/n): ")
SOLVING_STR =               ("Solving...")
IMPROPER_INPUT_STR =        ("Invalid input")
TOO_LOW_INPUT_STR =         ("Invalid input, number must be at least {}")
WEB_SCRAPE_ERROR =          ("Web Scraping Error. Exiting.")
LEFT_BTN_TEXT =             ("<")
RIGHT_BTN_TEXT =            (">")
ENTER_BTN_TEXT =            ("Enter")
NEW_PUZZLE_BTN_TEXT =       ("New puzzle")
GUI_SELECTED_TILE_COLOR =   ("white")
GUI_DESELECTED_TILE_COLOR = ("grey")